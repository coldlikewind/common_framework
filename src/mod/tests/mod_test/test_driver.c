...
#define TEST_DRIVER_UDPPORT 9001
...
void test_driver_listen6(agc_stream_handle_t *stream)
{
	...
	stream->write_function(stream, "add ipv6 connection ok.\n");
}
...
void test_driver_udp(agc_stream_handle_t *stream)
{
	...
	if (server_fd < 0) {
		agc_log_printf(AGC_LOG, AGC_LOG_ERROR, "udp socket failed .\n");
		stream->write_function(stream, "test_driver_udp failed.\n");
		return;
	}
	...
	servaddr.sin_family = AF_INET;
	inet_pton(AF_INET, TEST_DRIVER_LOCALADDR, &servaddr.sin_addr);
	servaddr.sin_port = htons(TEST_DRIVER_UDPPORT);	

	ret = bind(server_fd, (struct sockaddr*)&servaddr, sizeof(servaddr));

	if (ret < 0) {
		agc_log_printf(AGC_LOG, AGC_LOG_ERROR, "udp socket failed .\n");
		stream->write_function(stream, "test_driver_udp bind failed.\n");
		return;
	}
	...
	new_connection = agc_conn_create_connection(server_fd, 
	...
	stream->write_function(stream, "test udp connection ok.\n");
...
static void handle_read(void *data)
{
	...
	agc_driver_add_event(connection, AGC_WRITE_EVENT);
...
static void handle_udpread(void *data)
{
	...
	agc_log_printf(AGC_LOG, AGC_LOG_DEBUG, "read %s .\n", context->buf);
